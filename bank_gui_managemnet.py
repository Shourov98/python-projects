...
# function to return home menu
def home_return(master):
	master.destroy()
	Main_menu()

# Function of creating an account
def write(master, name, oc, pin):
	...
	messagebox.showinfo("Details", "You Account Number is :"+ str(accnt_no))
	master.destroy()
	return

# Logic of credit the account and show in gui
def crdt_write(master, amt, accnt, name):
	...
	fdet.write(str(strftime("[%Y-%m-%d]  [%H:%M:%S]		", gmtime()))+ "		"+str(amti)+"		"+str(cb)+"\n")
	frec.close()
...
# Logic of debit the account and show in gui
...
# Function To display Transection History Window
...
# Function to display create account display
def Create():
